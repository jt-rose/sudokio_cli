...
const displayGrid = sudokuGrid => {
    CP.rowSets.map( row => 
        _567‍.g.console.log( JSON.stringify(row.map(rowIndex => sudokuGrid[rowIndex]))));
};

// gradually display opening message, but show all puzzle names at once
openingMessages.forEach((msg, i) => {
    if (typeof msg === "string") {
        setTimeout( () => _567‍.g.console.log(msg), (700 * i))
    } else {
        setTimeout( () => msg.map(x => _567‍.g.console.log(x)), (700 * i))
    }
});
...
const followUpOptions = solutions => {
    ...
    prompt.get("option", function(err, result) {
        switch (result.option) {
            case "strategies":
                solutions.map( sol => _567‍.g.console.log(sol));
                console.log("");
                testUserGrid()
                break;
            case "next":
